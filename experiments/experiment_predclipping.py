...
with open('data/results/pred_clipping.csv', 'w') as file:
    file.write(',' + ','.join(map(str, range(num_runs))) + ',avg-runtime\n')

    # Pred clipping
    for pred_clipping in (2e-1, 1e-1, 5e-2, 2.5e-2, 1e-2, 5e-3, 2.5e-3, 1e-3, 1e-4, 1e-5):
        file.write(str(pred_clipping))
        ...
        for run in range(num_runs):
            ...
            model = GLN(
                ...
                num_classes=10, pred_clipping=pred_clipping
            )
            ...
            print(pred_clipping, run, accuracy)

        # Average runtime
        runtime = (time.time() - start) / num_runs
        print(pred_clipping, runtime)
